...
//   lists/{listId}              { name, storeId, createdBy, createdAt, updatedAt }
...
export async function deleteList(listId) {
  ...
  batch.delete(doc(db, "households", HOUSEHOLD_ID, "lists", listId));
  await batch.commit();
}

export async function renameList(listId, name) {
  await updateDoc(doc(db, "households", HOUSEHOLD_ID, "lists", listId), {
    name: name.trim(),
    updatedAt: serverTimestamp(),
  });
}

export async function setListStore(listId, storeId) {
  await updateDoc(doc(db, "households", HOUSEHOLD_ID, "lists", listId), {
    storeId: storeId || null,
    updatedAt: serverTimestamp(),
  });
...
export async function addItem(listId, rawName, user, opts = {}) {
  ...
  await updateDoc(doc(db, "households", HOUSEHOLD_ID, "lists", listId), {
    updatedAt: serverTimestamp(),
  }).catch(() => { /* list might be brand new with no fields yet; ignore */ });
...
export async function setItemDone(listId, itemId, done) {
...
export async function setItemSection(listId, itemId, normalized, section) {
  const batch = writeBatch(db);
